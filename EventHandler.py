...
from automation_script.extensions import extension_paths


def add_date_to_path(path: Path):
    
    dated_path = path / f'{date.today().year}' / f'{date.today().month}' / f'{date.today().day}'
...
def rename_file(source: Path, destination_path: Path):
    ...
    :param Path source: 
        source of file to be moved
    :param Path destination_path: 
...
class EventHandler(FileSystemEventHandler):
    ...
    def __init__(self, watch_patch: Path, destination_root: Path):
        self.watch_patch = watch_patch.resolve()
    ...
    def on_modified(self, event):
        ...
        """
        for child in self.watch_patch.iterdir():
